...
public class AsteroidSplitterImpl  {
    ...
    public void createSplitAsteroid(Entity e, World world) {
        ...
        if (life == 1) {
            radius = 6;
            speed = (float) Math.random() * 20f + 50f;
        } else if (life == 2) {
            radius = 10;
            speed = (float) Math.random() * 70f + 20f;
        } else if (life <= 0) {
        ...
        asteroid1.add(new MovingPart(0, 10000, speed, 0));
        ...
        asteroid2.add(new MovingPart(0, 10000, speed, 0));
